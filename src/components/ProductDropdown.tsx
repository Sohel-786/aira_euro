...
export function ProductDropdown({ isOpen, onClose, onMouseEnter, onMouseLeave }: ProductDropdownProps) {
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          {/* Dropdown Panel - Positioned below header with white background */}
          <motion.div
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            transition={{ duration: 0.3, ease: "easeOut" }}
            className="product-dropdown fixed left-0 right-0 bg-white z-[101] overflow-y-auto shadow-lg border-t border-primary-soft/30"
            style={{ 
              top: "130px",
              bottom: 0,
          ...
          >
            ...
            <div className="max-w-[1340px] mx-auto px-4 sm:px-6 lg:px-24 py-12">
